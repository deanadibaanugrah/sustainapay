    ...
    try {
      ...
      if (response.ok) {
        ...
        if (validToken) {
            ...
            setError(t.errFormat || 'Login berhasil, tapi format token dari backend tidak ditemukan.');
            console.error("Cek response backend kamu:", data);
        }
      } else {
        setError(data.message || t.errFail || 'Login gagal. Cek email & password kamu.');
      }
    } catch (err) {
      console.error(err);
      setError(t.errServer || 'Gagal terhubung ke server. Pastikan backend Laravel menyala.');
    } finally {
      setLoading(false);
    }
  };
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setError(data.message || t.errGoogleFail || 'Google Login gagal.');
        }
      } catch (err) {
        console.error(err);
        setError(t.errGoogleServer || 'Gagal terhubung ke server saat Google Login.');
      } finally {
        setLoading(false);
      }
    },
    onError: () => {
      setError(t.errGoogleCancel || 'Login Google dibatalkan atau gagal.');
    }
  });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAF9] p-6">
      <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl shadow-green-100 w-full max-w-md border border-gray-100 text-center">
        ...
        <h2 className="text-3xl font-extrabold text-gray-900 mb-8">{t.title || 'SustainaPay'}</h2>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            {loading ? (t.btnConnecting || 'Connecting...') : (t.btnSignIn || 'Sign In')}
          </button>
        </form>

        <div className="my-6 flex items-center justify-center">
          <span className="w-full border-t border-gray-200"></span>
          <span className="px-3 text-sm text-gray-400 bg-white">{t.or || 'OR'}</span>
          <span className="w-full border-t border-gray-200"></span>
        </div>
        ...
        >
          ...
          {t.btnGoogle || 'Sign in with Google'}
        </button>

        <p className="mt-6 text-sm text-gray-500">
          {t.noAccount || "Don't have an account?"} <Link to="/register" className="text-green-600 font-bold">{t.createAccount || 'Create New Account'}</Link>
        </p>
      </div>
    </div>
  );
